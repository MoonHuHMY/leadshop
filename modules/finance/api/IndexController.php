...
class IndexController extends BasicController
{
    ...
    private function remit($finance)
    {
        ...
        try {
            $subscribeData = [
                'money' => $finance->price,
                'serviceCharge' => $finance->service_charge,
                'type' => $finance->getTypeText2(),
            ];
            $subscribe = new PromoterWithdrawalSuccessMessage($subscribeData);
            //todo 设置页面
            \Yii::$app->subscribe->setUser($this->user->id)->setPage('pages/index/index')->send($subscribe);
        } catch (\Exception $exception) {
    ...
    private function reject($finance)
    {
        ...
        $extra['reject_content'] = $this->post['content'] ?? '拒绝打款';
        $finance->extra = to_json($extra);
        if (!$finance->save()) {
            Error($finance->getErrorMsg());
        }
        ...
        $user = User::find()
            ->where(['id' => $finance->UID, 'is_deleted' => 0])
            ->with('userInfo')->one();
        $desc = "分销商提现退回，退回佣金：{$finance->price}";
        \Yii::$app->currency->setUser($user)
            ->commission->refund(round($finance->price, 2), $desc);
